...
import { addUserEmail, checkMasUserExistsByEmail, createMasUserWithPassword, deactivateMasUser, deleteOauthLink , getMasUserByEmail, getOauthLinkBySubject, getUserEmail, oauthLinkExistsBySubject, reactivateMasUser } from '../../utils/mas-admin';
...
test.describe('Tchap : Login password', () => {
  ...
  test('register when account is deactivated reactivates account silently', 
    async ({ page, userData, screenChecker, startTchapRegisterWithEmail }) => {
    ...
    let verificationCode  = await getLatestVerificationCode(userData.email);
