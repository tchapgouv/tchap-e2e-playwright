...
test.describe('Create Room', () => {
  test('should allow us to create a public room with name', async ({ page, authenticatedUser }) => {
    ...
    await dialog.locator('.tc_TchapRoomTypeSelector_RadioButton_title').getByText('Forum').click();
    ...
    if (env === 'local') {
      ...
      await expect(page).toHaveURL(
        new RegExp(`/#/room/#test-room-public-1:${authenticatedUser.homeServer}`)
      );
  ...
  test('should allow us to create a private room with name', async ({
    ...
    if (env === 'local') {
      ...
      await expect(page).toHaveURL(
        new RegExp(`/#/room/#test-room-private-1:${authenticatedUser.homeServer}`)
      );
      const header = page.locator('.mx_RoomHeader');
  ...
  test('should allow us to create a private room with external with name', async ({
    ...
    if (env === 'local') {
      ...
      await expect(page).toHaveURL(
        new RegExp(`/#/room/#test-room-private-external-1:${authenticatedUser.homeServer}`)
      );
      const header = page.locator('.mx_RoomHeader');
